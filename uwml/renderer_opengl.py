...
class OpenGLRenderer(Renderer3DBase):
    # Màu sắc dễ nhìn
    C_BG   = (0.02, 0.02, 0.03, 1.0)
    C_GRID = (0.20, 0.20, 0.22, 1.0)
    C_BS   = (0.95, 0.95, 0.20, 1.0)
    C_UAV  = (0.20, 0.80, 1.00, 1.0)
    C_UE   = (0.20, 1.00, 0.20, 1.0)
    C_WIRE = (0.55, 0.70, 1.00, 1.0)
    C_AX_X = (1.00, 0.30, 0.30, 1.0)
    C_AX_Y = (0.30, 1.00, 0.30, 1.0)
    C_AX_Z = (0.30, 0.30, 1.00, 1.0)
    C_SAT  = (1.00, 0.50, 0.00, 1.0)

    def __init__(self, width=1280, height=760):
        self.width, self.height = width, height
        cfg = pyglet.gl.Config(double_buffer=True, depth_size=24, sample_buffers=1, samples=4)
        self.win = pyglet.window.Window(width, height, "UAV Wireless-ML (Phase 1)",
                                        config=cfg, resizable=True, vsync=True)

        # Camera mặc định to/đủ gần, dễ nhìn ngay khi mở
        self.yaw, self.pitch, self.dist = 30.0, 20.0, 250.0
        self.target = np.array([0.0, 0.0, 0.0], np.float32)

        # HUD
        self._hud = pyglet.text.Label(
            "", font_size=12, x=12, y=self.height-20, color=(255,255,255,255)
        )
        ...
        # --- FPS thủ công---
        self._fps_t0 = time.perf_counter()   # mốc thời gian tính FPS
        self._fps_frames = 0                 # đếm khung giữa 2 lần cập nhật
        self._fps_value = 0.0                # giá trị FPS hiển thị
        ...
        def on_key_press(sym, mod):
            if sym == key.ESCAPE:
                ...
                self.pitch = max(-80.0, min(80.0, self.pitch + 4.0))
            elif sym == key.DOWN:
                self.pitch = max(-80.0, min(80.0, self.pitch - 4.0))
            elif sym in (key.PLUS, key.NUM_ADD):
                self.dist = max(20.0, self.dist * 0.87)
            elif sym in (key.MINUS, key.NUM_SUBTRACT):
                self.dist = min(1500.0, self.dist / 0.87)
            elif sym == key.G:
        ...
        def on_mouse_scroll(x, y, sx, sy):
            if sy > 0:
                self.dist = max(20.0, self.dist * 0.9)
            else:
                self.dist = min(1500.0, self.dist / 0.9)

    def _auto_frame(self, world):
        pts = [world.bs.pos] + [a.pos for a in world.uav] + [u.pos for u in world.ue] + [s.pos for s in world.sat]
    ...
    def _begin_3d(self):
        ...
        # (Slim) Dùng fixed-function cho nhanh gọn
        GL.glMatrixMode(GL.GL_PROJECTION)
        GL.glLoadMatrixf(mvp.astype('float32'))
    ...
    def _update_hud(self, world):
        # --- FPS thủ công: cập nhật mỗi ~0.5s ---
        self._fps_frames += 1
    ...
    def _draw(self, world):
        ...
        wires = []
        for u in world.ue:
            dst = world._nearest_uav_pos(u.pos)
            wires.append((u.pos, dst))
        ...
        if sat_pts:
            self._draw_points(sat_pts, self.C_SAT, size_px=10.0)
